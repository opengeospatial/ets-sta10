...
public class Capability2Tests {
	...
	/**
	 * The list of ids for all the Things created during test procedure (will be used for
	 * clean-up)
	 */
	private List<Long> thingIds = new ArrayList<>();

	/**
	 * The list of ids for all the Locations created during test procedure (will be used
	 * for clean-up)
	 */
	private List<Long> locationIds = new ArrayList<>();

	/**
	 * The list of ids for all the HistoricalLocations created during test procedure (will
	 * be used for clean-up)
	 */
	private List<Long> historicalLocationIds = new ArrayList<>();

	/**
	 * The list of ids for all the Datastreams created during test procedure (will be used
	 * for clean-up)
	 */
	private List<Long> datastreamIds = new ArrayList<>();

	/**
	 * The list of ids for all the Observations created during test procedure (will be
	 * used for clean-up)
	 */
	private List<Long> observationIds = new ArrayList<>();

	/**
	 * The list of ids for all the Sensors created during test procedure (will be used for
	 * clean-up)
	 */
	private List<Long> sensorIds = new ArrayList<>();

	/**
	 * The list of ids for all the ObservedPropeties created during test procedure (will
	 * be used for clean-up)
	 */
	private List<Long> obsPropIds = new ArrayList<>();

	/**
	 * The list of ids for all the FeaturesOfInterest created during test procedure (will
	 * be used for clean-up)
	 */
	private List<Long> foiIds = new ArrayList<>();

	/**
	 * This method will be run before starting the test for this conformance class. It
	 * cleans the database to start test.
	 * @param testContext The test context to find out whether this class is requested to
	 * test or not
	 */
	@BeforeClass
	public void obtainTestSubject(ITestContext testContext) {
		...
		rootUri = testContext.getSuite().getAttribute(SuiteAttribute.TEST_SUBJECT.getName()).toString();
	...
	/**
	 * This method is testing create or POST entities. It only tests simple create, no
	 * deep insert. It makes sure that the response is 201 and use simple GET to make sure
	 * the entity is added to the service.
	 */
	@Test(description = "POST Entities", groups = "level-2", priority = 2)
	public void createEntities() {
		try {
			/* Thing */
			String urlParameters = "{" + "\"name\":\"Test Thing\","
			...
			// POST another Observation to make sure it is linked to the previously
			// created FOI
	...
	/**
	 * This method is testing create or POST in the form of Deep Insert. It makes sure the
	 * response is 201. Also using GET requests, it makes sure the entity and all its
	 * related entities are created and added to the service.
	 */
	...
	public void createEntitiesWithDeepInsert() {
		try {
			...
			deepInsertedObj = new JSONObject("{\n" + "        \"name\": \"Luminous Flux\",\n"
					...
					+ "        \"description\": \"Luminous Flux or Luminous Power is the measure of the perceived power of light.\"\n"
					+ "      },\n");
			obsPropIds.add(checkRelatedEntity(EntityType.DATASTREAM, datastreamId, EntityType.OBSERVED_PROPERTY,
					deepInsertedObj));
			thingIds.add(thingId);
			...
			deepInsertedObj = new JSONObject("{\n" + "          \"phenomenonTime\": \"2015-03-01T00:10:00.000Z\",\n"
					+ "          \"result\": 10\n" + "        }\n");
			observationIds
				.add(checkRelatedEntity(EntityType.DATASTREAM, datastreamId, EntityType.OBSERVATION, deepInsertedObj));
			datastreamIds.add(datastreamId);
			...
			deepInsertedObj = new JSONObject(
					"{\n" + "  \"name\": \"A weather station.\",\n" + "  \"description\": \"A weather station.\",\n"
							...
							+ "      \"type\": \"Point\",\n" + "      \"coordinates\": [\n" + "        -114.05,\n"
							+ "        51.05\n" + "      ]\n" + "    }\n" + "  }\n");
			foiIds.add(checkRelatedEntity(EntityType.OBSERVATION, obsId1, EntityType.FEATURE_OF_INTEREST,
					deepInsertedObj));
			observationIds.add(obsId1);
	...
	/**
	 * This method is testing create or POST with invalid Deep Insert. It makes sure that
	 * if there is any problem in the request body of Deep Insert, none of the entities in
	 * that query is created. The response should be 400 or 409 and the entities should
	...
	public void createInvalidEntitiesWithDeepInsert() {
		try {
			...
			// +
			// " \"description\": \"Luminous Flux or Luminous Power is the measure of the
	...
	/**
	 * This method is testing create or POST invalid entities. The response should be 400
	 * or 409 and the entity should not be accessible using GET.
	 */
	...
	public void createInvalidEntities() {
		try {
			...
			urlParameters = "{" + "\"name\":\"This is a Test Thing From TestNG\","
					+ "\"description\":\"This is a Test Thing From TestNG\"" + "}";
	...
	/**
	 * This method is testing partial update or PATCH. The response should be 200 and only
	 * the properties in the PATCH body should be updated, and the rest must be unchanged.
	 */
	...
	public void patchEntities() {
		try {
			...
			urlParameters = "{ \"unitOfMeasurement\": {\n" + "    \"name\": \"Entropy2\",\n"
					...
					+ "  } }";
			diffs = new HashMap<>();
			diffs.put("unitOfMeasurement", new JSONObject(
					"{\"name\": \"Entropy2\",\"symbol\": \"S2\",\"definition\": \"http://qudt.org/vocab/unit#Entropy2\"}"));
			updatedEntity = patchEntity(EntityType.DATASTREAM, urlParameters, datastreamId);
			checkPatch(EntityType.DATASTREAM, entity, updatedEntity, diffs);
	...
	/**
	 * This method is testing DELETE and its integrity constraint. The response should be
	 * 200. After DELETE the GET request to that entity should return 404.
	 */
	...
	/**
	 * This is helper method for checking the integrity containt of DELETE. For each
	 * entity, it checks after deleting, it confirm the deletion of its related entities
	 * mentioned in the integrity constraint of the specification.
	 */
	...
	// TODO: Add invalid PATCH test for other entities when it is implemented in the
	// service

	/**
	 * This method is testing invalid partial update or PATCH. The PATCH request is
	 * invalid if the body contains related entities as inline content.
	 */
	...
	public void invalidPatchEntities() {
		...
		// /** FeatureOfInterest **/
		// long foiId = foiIds.get(0);
		// urlParameters = "{\"feature\":{ \"type\": \"Point\", \"coordinates\": [114.05,
		// -51.05] }}";
	...
	/**
	 * This method is testing DELETE request for a nonexistent entity. The response should
	 * be 404.
	 */
	...
	/**
	 * This method created the URL string for the entity with specific id and then send a
	...
	/**
	 * This method created the URL string for the entity and then POST the entity with
	 * urlParameters to that URL.
	 * @param entityType Entity type in from EntityType enum
	...
	/**
	 * This helper method is sending invalid POST request and confirm that the response is
	 * correct based on specification.
	 * @param entityType Entity type in from EntityType enum
	...
	private void postInvalidEntity(EntityType entityType, String urlParameters) {
		...
		Map<String, Object> responseMap = HTTPMethods.doPost(urlString, urlParameters);
		int responseCode = Integer.parseInt(responseMap.get("response-code").toString());
		Assert.assertTrue(responseCode == 400 || responseCode == 409, "The  " + entityType.name()
				+ " should not be created due to integrity constraints. [Request] " + urlString);

	}

	/**
	 * This method created the URL string for the entity with specific id and then send
	...
	private void deleteEntity(EntityType entityType, long id) {
		...
		int responseCode = Integer.parseInt(responseMap.get("response-code").toString());
		Assert.assertEquals(responseCode, 200, "DELETE does not work properly for " + entityType + " with id " + id
				+ ". Returned with response code " + responseCode + ". [Request] " + urlString);

		responseMap = HTTPMethods.doGet(urlString);
		responseCode = Integer.parseInt(responseMap.get("response-code").toString());
		Assert.assertEquals(responseCode, 404,
				"Deleted entity was not actually deleted : " + entityType + "(" + id + "). [Request] " + urlString);
	}

	/**
	 * This method create the URL string for a nonexistent entity and send the DELETE
	 * request to that URL and confirm that the response is correct based on
	 * specification.
	 * @param entityType Entity type in from EntityType enum
	 */
	private void deleteNonExsistentEntity(EntityType entityType) {
		...
		Map<String, Object> responseMap = HTTPMethods.doDelete(urlString);
		int responseCode = Integer.parseInt(responseMap.get("response-code").toString());
		Assert.assertEquals(responseCode, 404, "DELETE does not work properly for nonexistent " + entityType
				+ " with id " + id + ". Returned with response code " + responseCode + ". [Request] " + urlString);

	}

	/**
	 * This method created the URL string for the entity with specific id and then PATCH
	 * the entity with urlParameters to that URL.
	 * @param entityType Entity type in from EntityType enum
	...
	private JSONObject patchEntity(EntityType entityType, String urlParameters, long id) {
		...
		try {
			...
			Assert.assertEquals(responseCode, 200,
					"Error during updating(PATCH) of entity " + entityType.name() + "with [Request] " + urlString);
			responseMap = HTTPMethods.doGet(urlString);
			JSONObject result = new JSONObject(responseMap.get("response").toString());
			return result;
	...
	/**
	 * This method created the URL string for the entity with specific id and then PATCH
	 * invalid entity with urlParameters to that URL and confirms that the response is
	 * correct based on specification.
	 * @param entityType Entity type in from EntityType enum
	...
	private void invalidPatchEntity(EntityType entityType, String urlParameters, long id) {
		...
		Map<String, Object> responseMap = HTTPMethods.doPatch(urlString, urlParameters);
		int responseCode = Integer.parseInt(responseMap.get("response-code").toString());
		Assert.assertEquals(responseCode, 400, "Error: Patching related entities inline must be illegal for entity "
				+ entityType.name() + ". [Request] " + urlString);
	 ...
	 * @param diffs The properties that supposed to be updated besed on the request due to
	 * the specification
	 */
		...
		try {
			for (String property : entityType.getProperties()) {
				if (diffs.containsKey(property)) {
					assertParameterEquals(property, newEntity.get(property).toString(), diffs.get(property).toString(),
							"PATCH was not applied correctly for " + entityType + "'s " + property + ".");
				}
				else {
					assertParameterEquals(property, newEntity.get(property).toString(),
							oldEntity.get(property).toString(),
							"PATCH was not applied correctly for " + entityType + "'s " + property + ".");
				}
			}
		}
	...
	/**
	 * Check the FeatureOfInterest is created automatically correctly if not inserted in
	 * Observation
	 * @param obsId The observation id
	 * @param locationObj The Location object that the FOI is supposed to be created based
	 * on that
	...
	private long checkAutomaticInsertionOfFOI(long obsId, JSONObject locationObj, long expectedFOIId) {
		...
		try {
			...
			if (expectedFOIId != -1) {
				Assert.assertEquals(id, expectedFOIId,
						"ERROR: the Observation should have linked to FeatureOfInterest with ID: " + expectedFOIId
								+ " , but it is linked for FeatureOfInterest with Id: " + id + ". [Request] "
								+ urlString);
			}
			Assert.assertEquals(result.getJSONObject("feature").toString(),
					locationObj.getJSONObject("location").toString(),
					"ERROR: Automatic created FeatureOfInterest does not match last Location of that Thing. [Request] "
							+ urlString);
			return id;
		}
		catch (JSONException e) {
	 ...
	 */
	private long checkRelatedEntity(EntityType parentEntityType, long parentId, EntityType relationEntityType,
			JSONObject relationObj) {
		boolean isCollection = true;
		String urlString = ServiceURLBuilder.buildURLString(rootUri, parentEntityType, parentId, relationEntityType,
				null);
		...
		try {
			Map<String, Object> responseMap = HTTPMethods.doGet(urlString);
			int responseCode = Integer.parseInt(responseMap.get("response-code").toString());
			Assert.assertEquals(responseCode, 200, "ERROR: Deep inserted " + relationEntityType
					+ " does not created or linked to " + parentEntityType + ".  [Request] " + urlString);
			JSONObject result = new JSONObject(responseMap.get("response").toString());
			...
			while (iterator.hasNext()) {
				...
				assertParameterEquals(key, Objects.toString(result.get(key)), Objects.toString(relationObj.get(key)),
						"ERROR: Deep inserted " + relationEntityType + " is not created correctly. [Request] "
								+ urlString);
			}
	...
	private void assertParameterEquals(String key, final String actual, final String expected, String message) {
		if (key.toLowerCase().contains("time") && !"null".equals(expected) && !"null".equals(actual)) {
			if (expected.contains("/")) {
				...
				Assert.assertEquals(ZonedDateTime.parse(actualParts[0]), ZonedDateTime.parse(expextedParts[0]),
						message);
				Assert.assertEquals(ZonedDateTime.parse(actualParts[1]), ZonedDateTime.parse(expextedParts[1]),
						message);
			}
			else {
	...
	private void checkForObservationResultTime(JSONObject observation, String resultTimeValue) {
		try {
			if (resultTimeValue == null) {
				Assert.assertEquals(observation.get("resultTime").toString(), "null",
						"The resultTime of the Observation " + observation.getLong(ControlInformation.ID)
								+ " should have been null but it is now \"" + observation.get("resultTime").toString()
								+ "\".");
			}
			else {
				Assert.assertEquals(ZonedDateTime.parse(observation.getString("resultTime")),
						ZonedDateTime.parse(resultTimeValue),
						"The resultTime of the Observation " + observation.getLong(ControlInformation.ID)
								+ " should have been \"" + resultTimeValue + "\" but it is now \""
								+ observation.get("resultTime").toString() + "\".");
			}
		}
		catch (JSONException e) {
			e.printStackTrace();
		...
		for (EntityType entityType : entityTypes) {
			...
			try {
				...
				Assert.assertEquals(array.length(), 0,
						entityType + " is created although it shouldn't. [Request] " + urlString);
			}
			catch (JSONException e) {
				e.printStackTrace();
				Assert.fail("An Exception occurred during testing!:\n" + e.getMessage() + " [Request] " + urlString);
	...
	private void createEntitiesForDelete() {
		...
		try {
			...
			urlString = ServiceURLBuilder.buildURLString(rootUri, EntityType.THING, thingIds.get(0),
					EntityType.LOCATION, null);
			responseMap = HTTPMethods.doGet(urlString);
			...
			// Observations
			urlString = ServiceURLBuilder.buildURLString(rootUri, EntityType.DATASTREAM, datastreamIds.get(0),
					EntityType.OBSERVATION, null);
			urlParameters = "{\n" + "  \"phenomenonTime\": \"2015-03-01T00:00:00Z\",\n" + "  \"result\": 1 \n" + "   }";
			responseMap = HTTPMethods.doPost(urlString, urlParameters);
			response = responseMap.get("response").toString();
			observationIds
				.add(Long.parseLong(response.substring(response.lastIndexOf("(") + 1, response.lastIndexOf(")"))));

			// FeatureOfInterest
			urlString = ServiceURLBuilder.buildURLString(rootUri, EntityType.OBSERVATION, observationIds.get(0),
					EntityType.FEATURE_OF_INTEREST, null);
			responseMap = HTTPMethods.doGet(urlString);
			response = responseMap.get("response").toString();
