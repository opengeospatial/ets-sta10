...
public class Capability1Tests {
	...
	/**
	 * The variable that defines to which recursive level the resource path should be
	 * tested
	 */
	...
	public void obtainTestSubject(ITestContext testContext) {
		...
		if ((null != obj)) {
			...
			Assert.assertTrue(level.intValue() > 0, "Conformance level 1 will not be checked since ics = " + level);
		}
	...
	/**
	 * This method is testing GET entities. It should return 200. Then the response
	 * entities are tested for control information, mandatory properties, and mandatory
	 * related entities.
	 */
	...
	/**
	 * This method is testing GET when requesting a nonexistent entity. The response
	 * should be 404.
	 */
	...
	/**
	 * This method is testing GET for a specific entity with its id. It checks the control
	 * information, mandatory properties and mandatory related entities for the response
	 * entity.
	 */
	...
	/**
	 * This helper method is testing property and property/$value for single entity of a
	 * given entity type
	...
	/**
	 * This helper method sending GET request for requesting a property and check the
	 * response is 200.
	 * @param entityType Entity type from EntityType enum list
	...
	private void checkGetPropertyOfEntity(EntityType entityType, long id, String property) {
		try {
			...
			Assert.assertEquals(responseCode, 200, "Reading property \"" + property + "\" of the existing "
					+ entityType.name() + " with id " + id + " failed.");
			...
			Assert.assertEquals(entity.length(), 1, "The response for getting property " + property + " of a "
					+ entityType + " returns more properties!");
		}
		catch (JSONException e) {
			e.printStackTrace();
			Assert.fail("An Exception occurred during testing!:\n" + e.getMessage());
	...
	/**
	 * This helper method sending GET request for requesting a property $value and check
	 * the response is 200.
	 * @param entityType Entity type from EntityType enum list
	 * @param id The id of the entity
	 * @param property The property to get requested
	...
	private void checkGetPropertyValueOfEntity(EntityType entityType, long id, String property) {
		...
		int responseCode = Integer.parseInt(responseMap.get("response-code").toString());
		Assert.assertEquals(responseCode, 200, "Reading property value of \"" + property + "\" of the exitixting "
				+ entityType.name() + " with id " + id + " failed.");
		String response = responseMap.get("response").toString();
		if (!property.equals("location") && !property.equals("feature") && !property.equals("unitOfMeasurement")) {
			Assert.assertEquals(response.indexOf("{"), -1,
					"Reading property value of \"" + property + "\"of \"" + entityType + "\" fails.");
		}
		else {
			Assert.assertEquals(response.indexOf("{"), 0,
					"Reading property value of \"" + property + "\"of \"" + entityType + "\" fails.");
		}
	}

	/**
	 * This method is testing the resource paths based on specification to the specified
	 * level.
	 */
	...
	/**
	 * This helper method is the start point for testing resource path. It adds the entity
	 * type to be tested to resource path chain and call the other method to test the
	 * chain.
	 * @param entityType Entity type from EntityType enum list
	 */
	...
	/**
	 * This helper method is testing the chain to the specified level. It confirms that
	 * the response is 200.
	...
	private void readRelatedEntity(List<String> entityTypes, List<Long> ids) {
		...
		try {
			...
			Assert.assertEquals(responseMap.get("response-code"), 200, "Reading relation of the entity from request: "
					+ urlString + " failed with type: " + entityTypes.toString());
			String response = responseMap.get("response").toString();
			Long id;
			if (isPlural) {
	...
	/**
	 * This method is checking the response for the request of Association Link. It
	 * confirms that it contains a list of selfLinks.
		...
		try {
			if (EntityType.isPlural(entityTypes.get(entityTypes.size() - 1))) {
				Assert.assertTrue(response.contains("value"),
						"The GET entities Association Link response does not match SensorThings API : missing \"value\" in response.: "
				...
				for (int i = 0; i < value.length() && count < 2; i++) {
					...
					try {
						Assert.assertNotNull(obj.get(ControlInformation.SELF_LINK),
								"The Association Link does not contain self-links.: " + entityTypes.toString()
										+ ids.toString());
					}
					catch (JSONException e) {
					...
					Assert.assertEquals(obj.length(), 1,
							"The Association Link contains properties other than self-link.: " + entityTypes.toString()
									+ ids.toString());
				}
			}
			else {
				...
				Assert.assertEquals(obj.length(), 1, "The Association Link contains properties other than self-link.: "
						+ entityTypes.toString() + ids.toString());
			}
		}
		catch (JSONException e) {
	...
	/**
	 * This method is check the response of sending a GET request to m=nonexistent entity
	 * is 404.
	 * @param entityType Entity type from EntityType enum list
	 */
	...
	/**
	 * This method is testing the root URL of the service under test. It basically checks
	 * the first page.
	 */
	...
	public void checkServiceRootUri() {
		try {
			...
			for (int i = 0; i < entities.length(); i++) {
				...
				switch (name) {
					case "Things":
						Assert.assertEquals(nameUrl, rootUri + "/Things",
						...
						break;
					case "Locations":
						Assert.assertEquals(nameUrl, rootUri + "/Locations",
								"The URL for Locations in Service Root URI is not compliant to SensorThings API.");
						addedLinks.remove("Locations");
						addedLinks.put(name, true);
						break;
					case "HistoricalLocations":
						Assert.assertEquals(nameUrl, rootUri + "/HistoricalLocations",
								"The URL for HistoricalLocations in Service Root URI is not compliant to SensorThings API.");
						addedLinks.remove("HistoricalLocations");
						addedLinks.put(name, true);
						break;
					case "Datastreams":
						Assert.assertEquals(nameUrl, rootUri + "/Datastreams",
								"The URL for Datastreams in Service Root URI is not compliant to SensorThings API.");
						addedLinks.remove("Datastreams");
						addedLinks.put(name, true);
						break;
					case "Sensors":
						Assert.assertEquals(nameUrl, rootUri + "/Sensors",
								"The URL for Sensors in Service Root URI is not compliant to SensorThings API.");
						addedLinks.remove("Sensors");
						addedLinks.put(name, true);
						break;
					case "Observations":
						Assert.assertEquals(nameUrl, rootUri + "/Observations",
								"The URL for Observations in Service Root URI is not compliant to SensorThings API.");
						addedLinks.remove("Observations");
						addedLinks.put(name, true);
						break;
					case "ObservedProperties":
						Assert.assertEquals(nameUrl, rootUri + "/ObservedProperties",
								"The URL for ObservedProperties in Service Root URI is not compliant to SensorThings API.");
						addedLinks.remove("ObservedProperties");
						addedLinks.put(name, true);
						break;
					case "FeaturesOfInterest":
						Assert.assertEquals(nameUrl, rootUri + "/FeaturesOfInterest",
								"The URL for FeaturesOfInterest in Service Root URI is not compliant to SensorThings API.");
						addedLinks.remove("FeaturesOfInterest");
						addedLinks.put(name, true);
						break;
					case "MultiDatastreams":
						Assert.assertEquals(nameUrl, rootUri + "/MultiDatastreams",
								"The URL for MultiDatastreams in Service Root URI is not compliant to SensorThings API.");
						break;
					default:
						Assert
							.fail("There is a component in Service Root URI response that is not in SensorThings API : "
									+ name);
						break;
				}
			}
	...
	private String getEntities(EntityType entityType) {
		...
		int responseCode = Integer.parseInt(responseMap.get("response-code").toString());
		Assert.assertEquals(responseCode, 200,
				"Error during getting entities: " + ((entityType != null) ? entityType.name() : "root URI"));
		if (entityType != null) {
			Assert.assertTrue(response.indexOf("value") != -1, "The GET entities response for entity type \""
					+ entityType + "\" does not match SensorThings API : missing \"value\" in response.");
		}
		else { // GET Service Base URI
			Assert.assertTrue(response.indexOf("value") != -1,
					"The GET entities response for service root URI does not match SensorThings API : missing \"value\" in response.");
		}
	...
	/**
	 * This helper method is the start point for checking the response for a collection in
	 * all aspects.
	 * @param entityType Entity type from EntityType enum list
	...
	/**
	 * This helper method is the start point for checking the response for a specific
	...
	/**
	 * This helper method is checking the control information of the response for a
	 * collection
	 * @param response The response of the GET request to be checked
	 */
		...
		try {
			...
			try {
				Assert.assertNotNull(entity.get(ControlInformation.ID),
						"The entity does not have mandatory control information : " + ControlInformation.ID);
			}
			catch (JSONException e) {
				Assert.fail("The entity does not have mandatory control information : " + ControlInformation.ID);
			}
			try {
				Assert.assertNotNull(entity.get(ControlInformation.SELF_LINK),
						"The entity does not have mandatory control information : " + ControlInformation.SELF_LINK);
			}
			catch (JSONException e) {
	...
	/**
	 * This helper method is checking the mandatory properties of the response for a
	 * collection
	 * @param entityType Entity type from EntityType enum list
	 * @param response The response of the GET request to be checked
	 */
	...
	private void checkEntityProperties(EntityType entityType, Object response) {
		try {
			...
			for (String property : entityType.getProperties()) {
				try {
					Assert.assertNotNull(entity.get(property), "Entity type \"" + entityType
							+ "\" does not have mandatory property: \"" + property + "\".");
				}
				catch (JSONException e) {
	...
	/**
	 * This helper method is checking the mandatory relations of the response for a
	 * collection
	 * @param entityType Entity type from EntityType enum list
	 * @param response The response of the GET request to be checked
	 */
			...
			for (String relation : entityType.getRelations()) {
				try {
					Assert.assertNotNull(entity.get(relation + ControlInformation.NAVIGATION_LINK),
							"Entity type \"" + entityType + "\" does not have mandatory relation: \"" + relation
